...
TInt CProviderTest::RunMethodL( 
    ...
    static TStifFunctionInfo const KFunctions[] =
        {  
         ...
         ENTRY( "CheckInputArgumentTypenos60prefix", CProviderTest::CheckInputArgumentTypenos60prefix ),
         ENTRY( "GetListManyTimes", CProviderTest::CheckGetListManyTimes ),
         ENTRY( "CheckOutputArgumentType", CProviderTest::CheckOutputArgumentType),
         ENTRY( "CheckCancel", CProviderTest::CheckCancel),
...
TInt CProviderTest::CheckGetListManyTimes( CStifItemParser& aItem  )
    {/* dummy */
    for (int i=0;i<6;i++)
    	{
    	int ret = Test(aItem);
...
TInt CProviderTest::CheckInputArgumentTypenos60prefix( CStifItemParser& aItem  )
	{
	iPrefix = ETrue;
	return CheckInputArgumentType(aItem);
	}


TInt CProviderTest::Test( CStifItemParser& aItem  )
	{
	//__UHEAP_MARK;
 	TPtrC16 serice16;
 	TPtrC16 Interface16;
  ...
  if (Interface && FillInputParmaeterList( aItem, input) == KErrNone )
    {
    //Call Service 
...
TInt CProviderTest::CheckInputArgumentType( CStifItemParser& aItem  )
  ...
  /*-------------------------------------------------------------------------------------------*/
	...
	if (command16.CompareF(_L("GetList")) == 0)
		iPrefix = ETrue;
   ...
   // PrintInputParameterList(input);
    ...
    command8.Copy(command16);  
     ...
     if(callback.CompareF(_L("NULL"))==0)
        ...
        
        {
         ...
         if(callback.CompareF(_L("LeavingCallBack"))==0)
            {
            callbackobj->iLeave = ETrue;  
            }
         //CleanupStack::PushL( callbackobj); 
         Interface->ExecuteCmdL(command8,*input,*output,servicecmdpptions, callbackobj); 
         //CleanupStack::PopAndDestroy( );
  ...
  else
    {
       ...
       // PrintInputParameterList(input);
        ...
        if(callback.CompareF(_L("checkinterface"))==0 && !Interface)
        	{
...
TInt CProviderTest::CheckOutputArgumentType( CStifItemParser& aItem  )
    {
     	...
     	if(err == 0 )
     	    {
     	     ...
     	     if( (callback != NULL) && (KLiwOptASyncronous == servicecmdpptions) )
                ...
                wait->Start();
     	        ...
     	        const TLiwGenericParam* p= NULL;
    			p = output->FindFirst(pos, KResponse); 
     	        if (!p)
     	        	{
     	        	err = KErrGeneral;	
     	        	}
...
TInt CProviderTest::CheckCancel( CStifItemParser& aItem  )
    {
     	...
     	if(err == 0 )
        {
     	     ...
     	     if( (callback != NULL) && (KLiwOptASyncronous == servicecmdpptions) )
			     ...
			     //	intValue = 0;
		            ...
		            valueParam.SetNameAndValueL(KTransactionID, value);
			        input->AppendL(valueParam);
			        
			        Interface->ExecuteCmdL(KCancel,*input,*output,KLiwOptCancel, NULL); 
        ...
        next8.Copy(next16);
      	...
      	if(dataType.CompareF(_L("Int") )== 0)
        ...
        else if ( dataType.CompareF(_L("String") ) == 0)
        	{
             ...
             if (next8.Compare(_L8("ApplicationID")) == 0)
        	 	{
        	 	if (!iPrefix)
        	 		value8.Insert(0,_L8("s60uid://"));	
        	 	}
        ...
        else if ( dataType.CompareF(_L("UnicodeString") ) == 0)
        	{
        	 ...
        	 if (next8.Compare(_L8("ApplicationID")) == 0 || 
        	 	next8.Compare(_L8("NULLApplicationID")) == 0 ||
        	 	next8.Compare(_L8("somethingelse")) == 0 )
        	 	{
        	 	if (!iPrefix)
        	 		temp.Insert(0,_L("s60uid://"));	
        	 	
        	 	if (next8.CompareF(_L8("NULLApplicationID")) == 0)             
             		next8.Copy(KNullDesC8);
